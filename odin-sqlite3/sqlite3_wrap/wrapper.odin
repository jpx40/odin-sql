...
sql_exec :: proc(db: ^DB, sql: string, args: ..any) -> (Status) {
    ...
    if db.blocking {
        ...
        query, status = sql_bind(db, sql, true,..args,)
...
_sql_bind :: proc(db: ^DB, sql: string, args: ..any) -> (^Query, Status) {
query: ^Query
unused: [^]u8
status: Status
...
sql_bind :: proc(db: ^DB, sql: string,  blockfree := false, args: ..any,) -> (^Query, Status) {
    if db.blocking && !blockfree {
         sync.lock(&db.mu) 
         defer sync.unlock(&db.mu) 
         return _sql_bind(db, sql, args)
...
sql_query :: proc(db: ^DB, sql: string,$T2: typeid,allocator :=context.allocator,args: ..any ) -> ([]T2, Status)
where
    intrinsics.type_is_struct(T2) {
    
    query : ^Query
    status : Status
    res := make([dynamic]T2,allocator=allocator)
    if db.blocking {
        if sync.guard(&db.mu) {
    ...
    for v in sql_row(db, query, T2,blockfree = true) {
        append(&res,v)
    }
    }
    } else {
...
_sql_row :: proc(db: ^DB, query: ^Query, $T: typeid) -> (T, bool)
where
    intrinsics.type_is_struct(T)
{
        ...
        #partial switch field_variant in field_base.variant {
            ...
            case runtime.Type_Info_String:
                ...
                if field_variant.is_cstring {
                    ...
                    (transmute(^cstring)  &t_bytes[field_offs])^ = strings.clone_to_cstring(v)
                } else {
                    v := cast(string) value
                    
                    (transmute(^string)  &t_bytes[field_offs])^ =  strings.clone(v)
                }
            case runtime.Type_Info_Array:
                if col_type != .Blob {
                    fmt.panicf("Type mismatch: %v <- %v", typeid_of(type_of(field_variant)), col_type)
